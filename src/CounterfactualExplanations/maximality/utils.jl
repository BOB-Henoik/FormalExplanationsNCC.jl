...
function fit_cf(m::Maximality, class_fitresult, x)
    ...
    dominance_thresholds::Dict{DominancePair, Float32} = Dict(p => compute_threshold(m, class_fitresult, x_coded, p.dominant, p.dominate) for p in pred.dominance_pairs)
    dominance_contrib::Dict{DominancePair, Vector{Float32}} = Dict(p => compute_contrib(m, class_fitresult, dominance_oppponents[p], x_coded, p.dominant, p.dominate) for p in pred.dominance_pairs)
    ...
    incomparability_thresholds::Dict{IncomparablePair, Vector{Float32}} = 
        Dict(p => [compute_threshold(m, class_fitresult, x_coded, p.class1, p.class2), 
                    compute_threshold(m, class_fitresult, x_coded, p.class2, p.class1)]
                for p in pred.incomparable_pairs)
    incomparability_contrib::Dict{IncomparablePair, Vector{Vector{Float32}}} = 
        Dict(p => [compute_contrib(m, class_fitresult, incomparability_oppponents[p][1], x_coded, p.class1, p.class2), 
                    compute_contrib(m, class_fitresult, incomparability_oppponents[p][2], x_coded, p.class2, p.class1)] 
                for p in pred.incomparable_pairs)
...
function fitted_params_cf(m::Maximality, fitresult)
    ...
    pred=fitted_decisions_cf(m, fitresult)
...
function fitted_decisions_cf(m::Maximality, fitresult)
